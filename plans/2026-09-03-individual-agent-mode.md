...
turn 输入的未读通知按同一条规则取值：通知顶部的未读总数覆盖该 actor 作用域内的全部对话线。
`session` 模式下作用域只有一条线，取值与今天一致；`dangerous_individual` 模式下它是该 Agent 的全部
对话线，与一次 `bcc message check` 排空的范围相同。总数与通知携带的消息是两个独立的量：总数来自
计数查询，通知按 target 逐行描述它这次带来的消息，行内条数不承诺等于该对话线的全部未读。
`_notice_for` 与 `_steer_active_turn` 共用的未读汇总据此按作用域计数，作用域由 actor 表达，不引入
模式判断。
...
turn 失败时的错误回执按同一个集合发：现在 `_runtime_loop` 只把 `batch[0].message` 交给
...
- 错误回执改为按同一个集合各发一次；
- 按第 2 节把未读汇总的计数改为覆盖 actor 作用域内的全部对话线，通知行仍只描述本次带来的消息；
- 补充测试覆盖：steer 后两条对话线各自出现标记且终态全部撤回、同一条线被 steer 两次时仍只有一个
  标记且能撤回、turn 失败时集合内每条线各收到一次回执且未参与的线不收到、个体模式下两条线各有未读
  时通知总数为两条线之和而通知行仍只覆盖被唤起的那条；
- 运行 focused tests 与全部静态门禁，停下等待 review。
