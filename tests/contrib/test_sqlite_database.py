...
async def test_sqlite_v27_names_a_dm_by_the_peer_it_belongs_to() -> None:
    ...
    async with aiosqlite.connect(database_path) as connection:
        ...
        # kana answered, so its DM exists twice; mika has not answered yet, so
        # the DM opened by handle is the only one it has
        await connection.executemany(
            "INSERT INTO channel_sessions ("
            "id, channel, provider_thread_id, target_kind, following, "
        ...
        # the side opened by handle has read further than the peer's own thread,
    ...
    try:
        async with database.reader() as session, session.transaction():
            sessions = await session.fetchall(
                "SELECT id, provider_thread_id, provider_identity_ref_json "
        ...
        # the half opened under a handle is gone, folded into the peer's own
        assert "channel-kana-handle" not in named
        # a chat that was never opened keeps the name that opens it
        renamed = next(row for row in sessions if row["id"] == "channel-mika-handle")
        assert '"delivery_handle":"mika"' in renamed[
            "provider_identity_ref_json"
        ].replace(" ", "")
        thread_ids = {row["id"] for row in threads}
        assert "thread-kana-chat" in thread_ids
        assert "thread-mika-handle" in thread_ids
