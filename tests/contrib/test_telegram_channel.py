...
async def test_telegram_lifecycle_identity_and_inbound_speaker_projection(
    ...
    try:
        ...
        # A bot is named by its own id too: a username opens a chat that does
        # not exist yet, which is a matter of delivery rather than of identity.
...
def test_telegram_reads_the_chat_a_send_landed_in() -> None:
    ...
    acknowledged = TelegramOutboundChannel._acknowledged_thread_id(
        {"message_id": 5, "chat": {"id": 7, "type": "private"}}, opened_by_name
    )
    assert acknowledged == "telegram:1:7:0"
