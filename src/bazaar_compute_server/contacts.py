...
class Contact:
    # the conversation as the node names it, which is what a read is asked with
    target: str
    thread_id: str
    actor_id: str
    kind: str
...
class Contacts:
    ...
    def edge(self) -> str | None:
        return str(self.offset + len(self.contacts)) if self.contacts else None
...
async def contacts(
    ...
    *,
    ...
    if answer is None:
        return Contacts(agent, (), offset, False, since, "silent")
    if not answer.get("ok"):
        return Contacts(agent, (), offset, False, since, "refused", answer.get("code"))
