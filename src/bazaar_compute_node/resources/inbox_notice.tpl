...
Upgrade available: bazaar-compute-node {{ upgrade_version }} (installed {{ installed_version }}). Mention it in passing when you reply and offer to upgrade; if the user agrees, run `bcc upgrade`. If they do not want it, just carry on.{% endif %}{% if closing_bracket_on_own_line %}
{% endif %}]{{ "" -}}
