...
class _Delivery:
    ...
    def receipt(self) -> Mapping[str, object]:
        ...
        thread_ids = tuple(
            ...
            for receipt in self.receipts
            if receipt.get("state") == "confirmed"
            and isinstance(receipt.get("provider_thread_id"), str)
        )
        return {
...
class TelegramOutboundChannel(TelegramApprovalChannel):
    ...
    def _acknowledged_thread_id(
        ...
        A chat opened by `@username` answers under its own numeric id from here
        on, and the send acknowledgement is the first place that id appears.
        """
